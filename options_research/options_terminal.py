# PIVOT: Create a dashboard that allows users to input one of the available tickers in the options_dataset.py file
# Problem: It is difficult to find good, high quality and reliable data for options for free (most datasets are paid); 
# Solution: Create a free options analytics engine that allows users to enter a stock or ETF and returns the equity's options data (that is downloadable)

# STEPS:
# 1. Ask users what stock/ETF they wish to analyze
# 2. With that input parameter, fetch data from the API with that stock/ETF only
# 3. Pull data out of PostgreSQL and into Python
# 4. Connect Python to HTML/CSS
# 5. Design an Exa.ai-style dashboard that allows users to fetch data from the given SQL dataset


# Commented out ↓ because we no longer need it, but might need it sometime soon
# from options_dataset import full_options_list
...
class Options_Chain:
    ...
    def front_month_put_options(self):
        ...
        if self.exp_date != None:
           ...
           call_contract = options.get_options_chain(symbol, self.exp_date)['calls']
           self.inject_options_data(call_contract, put_options_chain_id, symbol, self.exp_date)
    ...
    def front_month_put_extra(self, i, symbol, date, contract_name, put, strike, put_options_chain_id, volume, oi, impvol):
        ...
        day = int(splitted_date[i].replace(',', ''))
        days_until_expiration = day - self.current_day
    ...
    def inject_options_data(self, contract, buying_method, options_chain_id, symbol, date):
        ...
        # Get the length of the options chain (in rows) 
